...
var DiffuseShader = /** @class */ (function (_super) {
    __extends(DiffuseShader, _super);
    function DiffuseShader() {
        var _this = _super.call(this) || this;
        _this.setShaderName(DiffuseShader.ShaderName);
        _this.color = new Vector4(1, 0, 0, 1);
        _this.diffuseTexture = new Laya.SolidColorTexture2D(_this.color);
        _this.tilingOffset = new Vector4(1, 1, 0, 0);
        return _this;
    }
    Object.defineProperty(DiffuseShader.prototype, "diffuseTexture", {
        get: function () {
    ...
    Object.defineProperty(DiffuseShader.prototype, "color", {
        get: function () {
    ...
    Object.defineProperty(DiffuseShader.prototype, "tilingOffset", {
        get: function () {
    ...
    Object.defineProperty(DiffuseShader.prototype, "renderMode", {
        set: function (value) {
            switch (value) {
                case 0:
    ...
    DiffuseShader.initShader = function () {
        var attributeMap = {
        ...
        var n = Laya.Shader3D.nameKey.add(DiffuseShader.ShaderName);
        // Laya.ShaderCompile3D.add(n, "attribute vec4 a_Position;\n attribute vec2 a_Texcoord;\n attribute vec3 a_Normal;\n uniform mat4 u_MvpMatrix;\n uniform mat4 u_WorldMat;\n varying vec2 v_Texcoord;\n varying vec2 V_RefracTex;\n varying vec3 v_Normal;\n #ifdef BONE\n attribute vec4 a_BoneIndices;\n attribute vec4 a_BoneWeights;\n const int c_MaxBoneCount = 24;\n uniform mat4 u_Bones[c_MaxBoneCount];\n #endif\n #if defined(DIRECTIONLIGHT)\n varying vec3 v_PositionWorld;\n #endif\n void main(){\n     #ifdef BONE\n     mat4 skinTransform=mat4(0.0);\n     skinTransform += u_Bones[int(a_BoneIndices.x)] * a_BoneWeights.x;\n     skinTransform += u_Bones[int(a_BoneIndices.y)] * a_BoneWeights.y;\n     skinTransform += u_Bones[int(a_BoneIndices.z)] * a_BoneWeights.z;\n     skinTransform += u_Bones[int(a_BoneIndices.w)] * a_BoneWeights.w;\n     vec4 position = skinTransform * a_Position;\n     gl_Position=u_MvpMatrix * position;\n     mat3 worldMat=mat3(u_WorldMat * skinTransform);\n     #else\n     gl_Position=u_MvpMatrix * a_Position;\n     mat3 worldMat=mat3(u_WorldMat);\n     #endif\n     v_Texcoord=a_Texcoord;\n     v_Normal=worldMat*a_Normal;\n     #if defined(DIRECTIONLIGHT)\n     #ifdef BONE\n     v_PositionWorld=(u_WorldMat*position).xyz;\n     #else\n     v_PositionWorld=(u_WorldMat*a_Position).xyz;\n     #endif\n     #endif\n    }", "#ifdef FSHIGHPRECISION\n     precision highp float;\n     #else\n     precision mediump float;\n     #endif\n     #include 'LightHelper.glsl';\n     varying vec2 v_Texcoord;\n     uniform sampler2D u_texture;\n     uniform vec4 u_Color;\n     varying vec3 v_Normal;\n     #if defined(DIRECTIONLIGHT)\n     uniform vec3 u_CameraPos;\n     varying vec3 v_PositionWorld;\n     #endif\n     #ifdef FOG\n    uniform float u_FogStart;\n    uniform float u_FogRange;\n    #ifdef ADDTIVEFOG\n    #else\n    uniform vec3 u_FogColor;\n    #endif\n    #endif\n     void main(){\n         vec4 c=texture2D(u_texture,v_Texcoord);\n         gl_FragColor = c * u_Color;\n         #ifdef FOG\n		float lerpFact=clamp((1.0/gl_FragCoord.w-u_FogStart)/u_FogRange,0.0,1.0);\n		#ifdef ADDTIVEFOG\n			gl_FragColor.rgb=mix(gl_FragColor.rgb,vec3(0.0,0.0,0.0),lerpFact);\n		#else\n			gl_FragColor.rgb=mix(gl_FragColor.rgb,u_FogColor,lerpFact);\n		#endif\n	#endif\n}", attributeMap, uniformMap)
        var shaderCompile3D = Laya.ShaderCompile3D.add(n, "attribute vec4 a_Position;\nattribute vec2 a_Texcoord;\nattribute vec3 a_Normal;\nuniform mat4 u_MvpMatrix;\nuniform mat4 u_WorldMat;\nvarying vec2 v_Texcoord;\nvarying vec2 V_RefracTex;\nvarying vec3 v_Normal;\n#ifdef BONE\nattribute vec4 a_BoneIndices;\nattribute vec4 a_BoneWeights;\nconst int c_MaxBoneCount = 24;\nuniform mat4 u_Bones[c_MaxBoneCount];\n#endif\n#if defined(DIRECTIONLIGHT)\nvarying vec3 v_PositionWorld;\n#endif\n#ifdef TILINGOFFSET\nuniform vec4 u_TilingOffset;\n#endif\nvoid main(){\n#ifdef BONE\nmat4 skinTransform=mat4(0.0);\nskinTransform += u_Bones[int(a_BoneIndices.x)] * a_BoneWeights.x;\nskinTransform += u_Bones[int(a_BoneIndices.y)] * a_BoneWeights.y;\nskinTransform += u_Bones[int(a_BoneIndices.z)] * a_BoneWeights.z;\nskinTransform += u_Bones[int(a_BoneIndices.w)] * a_BoneWeights.w;\nvec4 position = skinTransform * a_Position;\ngl_Position=u_MvpMatrix * position;\nmat3 worldMat=mat3(u_WorldMat * skinTransform);\n#else\ngl_Position=u_MvpMatrix * a_Position;\nmat3 worldMat=mat3(u_WorldMat);\n#endif\nv_Texcoord=a_Texcoord;\n#ifdef TILINGOFFSET\nv_Texcoord=(vec2(v_Texcoord.x,v_Texcoord.y-1.0)*u_TilingOffset.xy)+u_TilingOffset.zw;\nv_Texcoord=vec2(v_Texcoord.x,1.0+v_Texcoord.y);\n#endif\nv_Normal=worldMat*a_Normal;\n#if defined(DIRECTIONLIGHT)\n#ifdef BONE\nv_PositionWorld=(u_WorldMat*position).xyz;\n#else\nv_PositionWorld=(u_WorldMat*a_Position).xyz;\n#endif\n#endif\n}", "#ifdef FSHIGHPRECISION\nprecision highp float;\n#else\nprecision mediump float;\n#endif\n#include 'LightHelper.glsl';\nvarying vec2 v_Texcoord;\nuniform sampler2D u_texture;\nuniform vec4 u_Color;\nvarying vec3 v_Normal;\n#if defined(DIRECTIONLIGHT)\nuniform vec3 u_CameraPos;\nvarying vec3 v_PositionWorld;\n#endif\n#ifdef FOG\nuniform float u_FogStart;\nuniform float u_FogRange;\n#ifdef ADDTIVEFOG\n#else\nuniform vec3 u_FogColor;\n#endif\n#endif\nvoid main(){\nvec4 c=texture2D(u_texture,v_Texcoord);\ngl_FragColor = c * u_Color;\n#ifdef FOG\nfloat lerpFact=clamp((1.0/gl_FragCoord.w-u_FogStart)/u_FogRange,0.0,1.0);\n#ifdef ADDTIVEFOG\ngl_FragColor.rgb=mix(gl_FragColor.rgb,vec3(0.0,0.0,0.0),lerpFact);\n#else\ngl_FragColor.rgb=mix(gl_FragColor.rgb,u_FogColor,lerpFact);\n#endif\n#endif\n}", attributeMap, uniformMap);
        DiffuseShader.SHADERDEFINE_TILINGOFFSET = shaderCompile3D.registerMaterialDefine("TILINGOFFSET");
    };
    DiffuseShader.COLOR = 0;
    DiffuseShader.DIFFUSETEXTURE = 1;
    DiffuseShader.TILINGOFFSET = 2;
    DiffuseShader.SHADERDEFINE_TILINGOFFSET = 0;
    DiffuseShader.ShaderName = "DiffuseShader";
